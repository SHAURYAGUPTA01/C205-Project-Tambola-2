...
def gameWindow():
    ...
    canvas2.create_text( screen_width/2.5,50, text = "Tambola Family Fun", font=("Chalkboard SE",50), fill="#3e2723")

    createTicket()
    placeNumbers()

    flashNumberLabel = canvas2.create_text(400,screen_height/2, text = "Waiting for other players to join...", font=("Chalkboard SE",30), fill="#3e2723")
...
def askPlayerName():
    ...
    canvas1.create_text( screen_width/3,screen_height/8, text = "Enter Name", font=("Chalkboard SE",60), fill="#3e2723")

    nameEntry = Entry(nameWindow, width=15, justify='center', font=('Chalkboard SE', 30), bd=5, bg='white')
    nameEntry.place(x = screen_width/6, y=screen_height/3 )

    button = tk.Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=11, command=saveName, height=2, bg="red", bd=3)
    button.place(x = screen_width/5, y=screen_height/2)
